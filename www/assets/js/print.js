console.log("printElement loaded");
(function ($) {
  $.fn.printElement = function (options) {
    ...
    if (settings.title != null && settings.title != "") {
      head = $(head).append(
        $(document.createElement("title")).text(settings.title)
      );
    } else {
      head = $(head);
    }
    ...
    if (settings.css == "extend" || settings.css == "style") {
      head.append(
        $(document.createElement("style")).append($("style").clone().html())
      );
    }
    ...
    // Хэрэглэгчийн өгсөн гарчиг байвал эхэнд нэмэх
    ...
    var fn_window = window.open(
      "",
      settings.title,
      "width=" + $(document).width() + ",height=" + $(document).width() + ""
    );
    // write html to new window
    fn_window.document.write($(html).prop("outerHTML"));
